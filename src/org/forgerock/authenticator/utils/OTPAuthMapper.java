...
public class OTPAuthMapper {
     ...
     * @throws URIMappingException If there was an unexpected error parsing.
     */
    public Map<String, String> map(URI uri) throws URIMappingException {
    ...
    private static String stripSlash(String s) {
        if (s.startsWith(SL)) {
            return stripSlash(s.substring(SL.length(), s.length()));
        }
        if (s.endsWith(SL)) {
            return stripSlash(s.substring(0, s.length() - SL.length()));
        }
        return s;
    }
}
